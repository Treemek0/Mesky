...
public class AlertsGui extends GuiScreen {
	...
	public void initGui() {
        ...
        for (int i = 0; i < Alerts.alertsList.size(); i++) {
        	...
        	// Delete button
        	this.deleteButtonList.add(new DeleteButton(0 + (10*i), (int)(width * 0.85f), inputFullPosition, inputHeight, inputHeight, ""));
    ...
    protected void actionPerformed(GuiButton button) {
		...
		if(button.id == -2) {
        	...
        	DeleteButton deleteButton = new DeleteButton(0 + (10*i), (int)(width * 0.85f), inputFullPosition, inputHeight, inputHeight, "");
        ...
        for (GuiButton guiButton : deleteButtonList) {
			if(guiButton.id == button.id) {
	    		...
	    		for (int i = 0; i < alertsFields.size(); i+=3) {
	    			...
	    			DeleteButton deleteButton = new DeleteButton(0 + (10*whichAlert), (int)(width * 0.85f), inputFullPosition, inputHeight, inputHeight, "");
	            	deleteButtonList.add(deleteButton);
	...
	private void SaveAlerts() {
	    ...
	    for (int i = 0; i < alertsFields.size(); i += 3) {
	        ...
	        boolean onlyParty = ((CheckButton)checksButtonList.get(i/3)).isFull();
	        boolean ignorePlayers = ((CheckButton)checksButtonList.get((i/3)+1)).isFull();
	        boolean isEqual = ((CheckButton)checksButtonList.get((i/3)+2)).isFull();
	        alertsList.add(new Alert(trigger, display, time, onlyParty, ignorePlayers, isEqual));
	    }
