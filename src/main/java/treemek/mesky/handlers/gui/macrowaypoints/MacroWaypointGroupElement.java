...
public class MacroWaypointGroupElement {
	String name;
	List<MacroWaypointElement> list;
	...
	public void updateYposition(int y, int inputHeight, int inputMargin, MacroWaypointElement holdingElement) {
		this.yPosition = y;
		this.yPositionD = y;
		...
		if(opened.isOpened()) {
			...
			for (MacroWaypointElement waypointElement : this.list) {
				if(holdingElement != waypointElement) waypointElement.updateYposition(y, inputHeight);
				y += waypointHeight + inputMargin;
				h += waypointHeight + inputMargin;
			}
		}
	...
	public void updateYpositionSmooth(int y, int inputHeight, int inputMargin, MacroWaypointElement holdingElement) {
		...
		h += inputHeight + inputMargin/2;
		if(opened.isOpened()) {
			for (MacroWaypointElement element : list) {
				if (element != holdingElement) {
					if(yPosition == orgY) {
						...
						if(Math.abs(l) > Math.abs(element.yPositionD - y)) {
							element.updateYposition(y, inputHeight);
						}else {
							element.updateYposition(element.yPositionD + l, inputHeight);
						}
					}else {
						element.updateYposition(yPosition + h + inputMargin/2, inputHeight);
					}
				}
