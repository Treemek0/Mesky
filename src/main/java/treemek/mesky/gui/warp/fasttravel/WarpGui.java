...
public class WarpGui extends GuiScreen{
	...
	public void initGui() {
		...
		private_island.enabled = unlocked_map.getOrDefault(FastTravel.PRIVATE_ISLAND, PadLock.LOCKED);
		...
		jerry_island.enabled = unlocked_map.getOrDefault(FastTravel.JERRYS_WORKSHOP, PadLock.LOCKED);
		...
		galatea.enabled = unlocked_map.getOrDefault(FastTravel.GALATEA, PadLock.LOCKED);
		islands.add(galatea);
		...
		park.enabled = unlocked_map.getOrDefault(FastTravel.THE_PARK, PadLock.LOCKED);
		int[][] parkPoints = {
			    {30, 35},
			    {100, 5},
			    {160, 110},
		...
		hub.enabled = unlocked_map.getOrDefault(FastTravel.SKYBLOCK_HUB, PadLock.LOCKED);
		hub.scaleOnHover = 1.1;
		int[][] hubPoints = {
		    {50, 30},
		    {120, 30},
		    {220, 5},
		...
		spiders_den.enabled = unlocked_map.getOrDefault(FastTravel.SPIDERS_DEN, PadLock.LOCKED);
		...
		bayou.enabled = unlocked_map.getOrDefault(FastTravel.BACKWATER_BAYOU, PadLock.LOCKED);
		islands.add(bayou);
		...
		barn.enabled = unlocked_map.getOrDefault(FastTravel.THE_BARN, PadLock.LOCKED);
		islands.add(barn);
		...
		gold_mine.enabled = unlocked_map.getOrDefault(FastTravel.GOLD_MINE, PadLock.LOCKED);
		...
		farming_island.enabled = unlocked_map.getOrDefault(FastTravel.THE_BARN, PadLock.LOCKED);
		...
		end.enabled = unlocked_map.getOrDefault(FastTravel.THE_END, PadLock.LOCKED);
		islands.add(end);
		...
		crimson_isle.enabled = unlocked_map.getOrDefault(FastTravel.CRIMSON_ISLE, PadLock.LOCKED);
		crimson_isle.scaleOnHover = 1.1;
		islands.add(crimson_isle);
		
		WarpIsland deep_caverns = new WarpIsland(gold_mine.xPosition + 90*p, spiders_den.yPosition - 80*p, 135*p, 160*p, "Dwarves mines", new ResourceLocation(Reference.MODID, "gui/warp/dwarves_mines.png"), Arrays.asList(new WarpPortal(40*p, 90*p, "/warp deep", "Deep Caverns"), new WarpPortal(115*p, 50*p, "/warp dwarves", "Dwarves mines", unlocked_map.getOrDefault(FastTravel.DWARVEN_MINES, PadLock.LOCKED).isUnlocked()), new WarpPortal(115*p, 90*p, "/warp forge", "Forge", unlocked_map.getOrDefault(FastTravel.DWARVEN_MINES, PadLock.LOCKED).isUnlocked()), new WarpPortal(85*p, 65*p, "/warp basecamp", "Base camp", unlocked_map.getOrDefault(FastTravel.DWARVEN_MINES, PadLock.LOCKED).isUnlocked()), new WarpPortal(100*p, 115*p, "/warp crystal", "Crystal Hollows", unlocked_map.getOrDefault(FastTravel.CRYSTAL_HOLLOWS, PadLock.LOCKED).isUnlocked()), new WarpPortal(75*p, 140*p, "/warp nucleus", "Nucleus", unlocked_map.getOrDefault(FastTravel.CRYSTAL_HOLLOWS, PadLock.LOCKED).isUnlocked())));
		deep_caverns.enabled = unlocked_map.getOrDefault(FastTravel.DEEP_CAVERNS, PadLock.LOCKED);
		deep_caverns.scaleOnHover = 1.3;
		...
		dungeon_hub.enabled = unlocked_map.getOrDefault(FastTravel.DUNGEON_HUB, PadLock.LOCKED);
		...
		rift.enabled = unlocked_map.getOrDefault(FastTravel.RIFT_DIMENSION, PadLock.LOCKED);
		islands.add(rift);
	...
	protected void mouseReleased(int mouseX, int mouseY, int state) {
		for (WarpIsland warpIsland : islands) {
			warpIsland.mouseReleased(mouseX, mouseY);
		}
	}
